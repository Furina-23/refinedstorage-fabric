...
public class InvWrapper implements IItemHandler {
    ...
    public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        if (!isItemValid(slot, stack) || !container.canPlaceItem(slot, stack)) {
            return stack;
        }

        ItemStack current = getStackInSlot(slot);
        int limit = Math.min(getSlotLimit(slot), stack.getMaxStackSize());
        int amount = Math.min(limit - current.getCount(), stack.getCount());
        if (amount <= 0 || (!current.isEmpty() && !ItemStack.isSameItemSameTags(current, stack))) {
            return stack;
        }

        if (!simulate) {
            ItemStack inserted = current.isEmpty() ? stack.copyWithCount(amount) : current.copy();
            if (!current.isEmpty()) {
                inserted.grow(amount);
            }
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!simulate) {
            container.removeItem(slot, result.getCount());
        }
        return result;
    }
